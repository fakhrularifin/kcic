...
<body>
        ...
        <!-- main content -->
			<div class="main-content">
				...
				<div class="container-fluid">
					<!-- page -->
					<h3 class="page-title">List Surat Masuk</h3>
									...
									<div class="panel-body">
										<div class="table-responsive">
											<table class="table table-hover" id="dataTable">
												<thead class="thead-light" style="text-height:bold; font-size:16px">
													<tr>
														<th></th>
														<th>Jenis Surat</th>
														<!-- <th>Nomor Surat</th> -->
					...
					$('#dataTable').DataTable({
						...
						"columnDefs": [ 
							...
							initComplete: function () {
								// $( this.api().columns().header() ).addClass( 'text-center' );
	...
	//fq_20201008: end new logic getsurat	
			...
			console.log(surat);
			let tglSurat = surat.tanggalSurat != 'Invalid date' ? surat.tanggalSurat : '';
			$('#modalDetailSurat #action').html('');
			$('#modalDetailSurat #attachments').html('');
			
			$('#modalDetailSurat #detailSuratTitle').html(`
				<small class='text-muted'><i class='fa fa-calendar'></i> ${tglSurat}</small>
				<h4 class='no-margin'>${surat.nomorSurat === null ? surat.jenisSurat.replace(/_|-/g, " ") : surat.nomorSurat}</h4>
				
				<br>
				<span class='label label-primary'>${surat.jenisSurat.replace(/_|-/g, " ")}</span> ${surat.sifat === 'NORMAL' ? '<span class="label label-success">NORMAL</span>' : '<span class="label label-danger">URGENT</span>'}&nbsp;&nbsp;&nbsp;&nbsp;
				...
				$.ajax({
					...
					success: function (response){ 
						...
						if(surat.pemeriksa === MAIL){
							console.log('surat :: ', surat.statusDisetujui);
				...
				$.ajax({
					...
					success: function (response){ 
						...
						$('#modalDetailSurat #detailSurat').html(`
							<div class='list-disposisi'>
								<table class='small'>
									...
									<tr>
										<td>Kepada&nbsp;</td>
										<td>&nbsp;:&nbsp;</td>
										<td>&nbsp;${response.data.penerima}</td>
									</tr>
									<tr>
										<td>CC&nbsp;</td>
										<td>&nbsp;:&nbsp;</td>
										<td>&nbsp;${response.data.penerimaCc === null ? '' : response.data.penerimaCc }</td>
									</tr>							
									<tr>
										<td>Perihal&nbsp;</td>
										<td>&nbsp;:&nbsp;</td>
										<td>&nbsp;<b><u>${response.data.perihal }</u></b></td>
									</tr>							
								</table>
								<hr/>
								${response.data.memo}
							...
							<div class='list-disposisi' style='margin-left:60px'>
								
								<div class='list-disposisi-pengirim' id='disposisi-pengirim'>
									<div class='text-muted' style='margin-bottom:5px;'><small><i class='fa fa-reply'></i> Balasan Disposisi</small></div>
									<table class='small' data-toggle='collapse' data-target='#disposisi-pengirim'>
										<tr>
											<td>Dari&nbsp;</td>
											<td>&nbsp;:&nbsp;</td>
										...
										<tr>
											<td>CC&nbsp;</td>
											<td>&nbsp;:&nbsp;</td>
											<td>&nbsp;${surat.penerimaCc === null ? '' : surat.penerimaCc}</td>
										</tr>							
									</table>
									<hr/>
									${surat.memo}
						...
						$('#modalDetailSurat #action').html(`
							<button class="btn btn-info pull-right" id="btnBuatBalasanDisposisi" data-id="${surat.idSuratRef}"><i class="fa fa-edit"></i> Buat Balasan Disposisi</button> 
							<button class="btn btn-secondary" id="btnSummaryDisposisi" data-id="${surat.idSuratRef}"><i class="fa fa-search"></i> Lihat Summary Disposisi</button>&nbsp;
						`);		
					},
					error: function (error) {
						Swal.fire({
		...
		$(document.body).on('click', '#btnBuatRevisiSurat', function(){
			...
			$.ajax({
				...
				success: function (response){ 
					...
					$('#modalBuatSurat #penerimaCc').val(suratDetail.penerimaCc);
				...
				success: function (response){ 
					...
					$('#disposisiSummary').html(`
						<div class="panel">
							<div class="panel-body">
								<div class="d-flex justify-content-between">
								<div><button class="btn close" id="btnTutupDisposisiSummary"><i class="fa fa-times"></i></button></div>
								<h4 style="margin-top:0;">Summary Disposisi</h4>
								</div>
								<hr/>
								<div class="row">
									<div class="col-md-12"><strong>Catatan : </strong><br> ${memo}</div>
									<div class="col-md-12"><strong>Diteruskan Kepada : </strong><br> ${diteruskanKepada}</div>
								</div>
							</div>
						</div>
						<a href="${ 'http://api.pirantipandawa5.com:3099/api/surat/getDisposisiSummaryPdf/'+id }">Download summary disposisi pdf</a> <br>
						<hr/>
					`);
				},
				error: function(error){
